...
export class AuditService {
  ...
  static async log(ctx: AuditContext) {
    ...
    const actor: AuditActor = {
      ...
      ipAddress: ctx.ipAddress ?? null as any,
      userAgent: ctx.userAgent ?? null as any,
    };
    ...
    const entryHash = computeHash({
      ...
      ipAddress: ctx.ipAddress ?? null as any,
      userAgent: ctx.userAgent ?? null as any,
      requestId: ctx.requestId ?? null as any,
    ...
    const record = await prisma.auditLog.create({
      data: {
        ...
        snapshotBefore: (ctx.payloadBefore ?? {}) as any,
        snapshotAfter: (ctx.payloadAfter ?? {}) as any,
        details: { ...ctx.details ?? {}, _status: status, _role: actor.role } as any,
        entryHash,
        previousHash,
        anchoredAt: null as any,
        anchorTxId: null as any,
        redactedAt: null as any,
      },
    });
  ...
  static async verifyIntegrity(limit = 100_000): Promise<IntegrityReport> {
    ...
    for (const log of logs) {
      ...
      const recomputed = computeHash({
        userId: log.adminId,
        role: (details._role as string) ?? 'SYSTEM',
        action: log.action,
        status: (details._status as any) ?? AuditStatus.SUCCESS,
        targetType: log.targetType,
        targetId: log.targetId,
        ipAddress: (log.ipAddress ?? undefined) as any,
        userAgent: (log.userAgent ?? undefined) as any,
        requestId: (log.requestId ?? undefined) as any,
        correlationId: (log.correlationId ?? undefined) as any,
        payloadBefore: (log.snapshotBefore ?? {}) as any,
        payloadAfter: (log.snapshotAfter ?? {}) as any,
