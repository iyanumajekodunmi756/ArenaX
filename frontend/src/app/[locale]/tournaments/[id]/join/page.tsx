...
import {
  ...
  TOURNAMENT_JOIN_BANNER_SIZES,
} from "@/lib/tournamentImageSizes";
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background px-4">
        <div className="text-center">
          <p className="text-lg font-medium text-foreground">Loading tournament...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background px-4 py-8">
      <div className="mx-auto max-w-lg">
        ...
        <div className="relative mb-6 h-48 w-full overflow-hidden rounded-lg bg-muted">
          <Image
            src={resolveTournamentBanner(tournament)}
            alt={`${tournament.name} banner`}
            fill
            sizes={TOURNAMENT_JOIN_BANNER_SIZES}
            className="object-cover"
          />
        </div>

        <div className="mb-6">
          <h1 className="text-2xl font-bold text-foreground">Join Tournament</h1>
          <p className="mt-1 text-muted-foreground">
            Confirm your entry for{" "}
